...
def parse_args() -> argparse.Namespace:
    ...
    # MODIFICATION: Added thresholds to tune from the command line
    ap.add_argument(
        "--viz-score-thresh", type=float, default=0.25,
        help="Score threshold for visualization (inference_loop)."
    )
    ap.add_argument(
        "--eval-score-thresh", type=float, default=0.05,
        help="Score threshold for manual evaluation (--eval-backend manual)."
    )
...
def main() -> None:
    ...
    if args.eval and args.eval_backend == "manual":
        ...
        run_manual_benchmark(
            ...
            score_thresh=args.eval_score_thresh,       # MODIFIED: Now from CLI
        ...
        metrics = inference_loop(
            ...
            out_dir=args.out_dir,
            device=args.device,
            score_thresh=args.viz_score_thresh,                 # MODIFIED: Visualization threshold from CLI
            metrics=metrics,
